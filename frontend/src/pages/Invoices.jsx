  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addInvoice({
        ...
        items: form.items.map(i => ({ ...i, productId: Number(i.productId), qty: Number(i.qty), price: Number(i.price), total: Number(i.total) })),
        subtotal,
...
export default function Invoices() {
  const { invoices, fetchInvoice } = useApp();
  ...
  const rows = invoices.map(inv => [
    ...
    <button onClick={() => handleView(inv)} className="text-xs text-[#6c63ff] hover:text-[#7c75ff] font-medium border border-[#6c63ff]/30 px-2 py-1 rounded-lg hover:bg-[#6c63ff]/10 transition-all">View</button>,
  ]);
